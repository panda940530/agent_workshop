...
try:
    llm = get_llm()
    r = llm.invoke("ping")
    print(f"✅ LLM 連線成功: {type(r.content)}")
...
try:
    ...
    print(f"❌ Helper 驗證失敗: {e}")

print("--- 驗證完成 ---")
'@
$script | Out-File -FilePath "_test_tmp.py" -Encoding utf8
python _test_tmp.py
Remove-Item "_test_tmp.py"

# 建立暫時測試檔(Mac OS)
# 1. 建立暫時的 python 檔
cat << 'EOF' > _test_tmp.py
from dotenv import load_dotenv
import os
load_dotenv()
from graph.nodes import get_llm, _extract_text, _extract_json
...
print("--- 驗證完成 ---")
EOF
# 2. 執行腳本 (Mac 建議用 python3)
python3 _test_tmp.py
# 3. 刪除暫存檔
rm _test_tmp.py
