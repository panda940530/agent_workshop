...
# 驗證內容
cuisine = result['preferences']['cuisine_type']
assert cuisine == 'japanese', f'Expected japanese, got {cuisine}'
...
$script | Out-File -FilePath "_test_nodes.py" -Encoding utf8
python _test_nodes.py
Remove-Item "_test_nodes.py"

## 驗證(Mac OS)

# 1. 建立測試腳本
cat << 'EOF' > _test_nodes.py
from dotenv import load_dotenv
import os
import sys
# 載入環境變數
...
try:
    state = {'user_input': '我想吃日式料理，預算 200 元'}
    result = parse_input_node(state)
    print(f"✅ parse_input: {result['preferences']}")
    # 驗證內容 (LLM 回傳可能是 japanese)
    cuisine = result['preferences'].get('cuisine_type', '').lower()
    assert 'japanese' in cuisine or '日' in cuisine, f'Expected japanese, got {cuisine}'
    # 測試 recommend_node
    state['preferences'] = result['preferences']
    state['budget_level'] = 'budget'
    rec_result = recommend_node(state)
    recs = rec_result.get('recommendations', [])
    print(f"✅ recommend: 找到 {len(recs)} 間餐廳")
    print("\n--- All nodes OK! ---")
except Exception as e:
    print(f"❌ 測試失敗: {e}")
    sys.exit(1)
EOF
# 2. 執行 (Mac 建議用 python3)
python3 _test_nodes.py
# 3. 刪除暫存檔
rm _test_nodes.py
